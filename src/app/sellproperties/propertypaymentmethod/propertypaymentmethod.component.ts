...
@Component({
  selector: 'app-propertypaymentmethod',
  templateUrl: './propertypaymentmethod.component.html',
  styleUrls: ['./propertypaymentmethod.component.scss']
})
export class PropertypaymentmethodComponent implements OnInit {
  ...
  searchResult: any;
  walletid:any;
  loading: boolean = false; hasErrorMesage: boolean = false; hasMessage: boolean = false; displayMessage: string;
  ...
  // addcard() {
  //   this.loading = true;
  //   this.hasErrorMesage = this.hasMessage = false; this.displayMessage = null;
  //   this.wallet.AccountNumber = this.num;
  //   this.wallet.CardType = this.type;
  //   this.wallet.UserId = this.currentUser.Id;
  //   this.userservice.saveUserWalletInfo(this.wallet).subscribe(res => {
  //     this.response = res;
  //     {
  //       this.displayMessage = "Card saved Successfully!";
  //       this.loading = this.hasErrorMesage = false;
  //       this.hasMessage = true;
  //       this.getuserwallet(this.userdetails.Id)

  //       $('#addCardModal').modal('hide');

  //       window.scrollTo(0, 0);
  //     }  {
  //       this.displayMessage = this.response.ErrorMessage;
  ...
  confirmpayment() {
    this.router.navigate(['/propertydetails/promoteemail',this.propertyId]);
  }
